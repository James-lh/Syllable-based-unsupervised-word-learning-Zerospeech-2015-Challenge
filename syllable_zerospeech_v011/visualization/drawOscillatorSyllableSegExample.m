function drawOscillatorSyllableSegExample(original_audio,bounds,envelopes,osc_env)
...
osc = osc_env{1}(timeinds(1):timeinds(end));
osc = osc-min(osc);
osc = osc./max(osc).*max(envelopes{1}(timeinds(1):timeinds(end)));
plot(t_env,osc+0.1,'LineWidth',1,'Color','black','LineWidth',2,'LineStyle','--');hold on;
a = find(bounds{1} > timeinds(1));
b = find(bounds{1} < timeinds(end));
c = intersect(a,b);
bb = bounds{1}(c);
