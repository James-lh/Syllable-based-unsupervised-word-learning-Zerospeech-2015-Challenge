function [vq_labels,weights,quant_error] = getLabels(F,codebook,n_of_streams)
% function [vq_labels,weights,quant_error] = getLabels(F,codebook,n_of_streams)
...
for b = 1:N
    ...
    segment_data = F{b};   
    inputti = zeros(size(codebook,1),size(segment_data,1));
    mm = 1;
    ...
    M = abs(pdist2(codebook',inputti','euclidean'));
